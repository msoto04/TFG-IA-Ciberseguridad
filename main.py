...
def ejecutar_auditoria_completa(ruta):
    ...
    for i, h in enumerate(hallazgos):
        print(f"-> Procesando hallazgo {i+1}/{len(hallazgos)}: {h['vulnerabilidad']}...")
        
       
        resultado = app.invoke({"hallazgos_tecnicos": [h]})
        
        tiempos = resultado['tiempos']
        print(f"    [Reloj] T. Técnico: {tiempos['tecnico']}s | T. Legal: {tiempos['legal']}s")
    ...
    try:
        ...
        with open(ruta_md, "w", encoding="utf-8") as f:
            f.write(f"# INFORME TÉCNICO-LEGAL DE SEGURIDAD (ENS)\n")
            f.write(f"**Fecha:** {os.popen('date').read().strip() if os.name != 'nt' else ''}\n")
